...
const createOverallData = (
  data: { labels: string[]; data: string[]; slug: string }[]
) => {
...
const CategoryButton = ({
  ...
  return (
    <button
      className={`font-basement w-[200px] p-2 rounded-lg transition-all duration-200 flex items-center justify-center ${
        isActive
          ? "transform scale-105 shadow-lg text-white"
          : `${categoryColor.button} hover:bg-white transition-colors`
      }`}
...
const SkillCard = ({
  ...
  >
    ...
    <div className="mt-2">
      <div
        className={`w-full rounded-full h-2 ${isDarkMode ? "bg-gray-600" : "bg-gray-200"}`}
      >
        <div
...
const StatisticsCard = ({
  ...
  const averageSkill =
    skillsData.length > 0
      ? (
          skillsData.reduce((sum, skill) => sum + skill.value, 0) /
          skillsData.length
        ).toFixed(1)
      : "0";
