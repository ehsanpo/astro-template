    ...
    >
      ...
      <div>
        <img
          ...
          style={{
            viewTransitionName: `portfolio-image-${
              item.permalink || item.slug
            }`,
          }}
        />
      </div>
      <div className="pointer-events-none z-25 absolute inset-0 flex items-center justify-center bg-black/0 opacity-0 transition-all duration-300 group-hover:bg-black/40 group-hover:opacity-100">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 p-4">
        <div className="mb-3 flex items-center gap-4">
          <div>
            <a
              href={`/${item.type === "product" ? "products" : "portfolio"}/${
                item.permalink || item.slug
              }`}
            >
              <h3
                ...
                style={{
                  viewTransitionName: `portfolio-title-${
                    item.permalink || item.slug
                  }`,
                }}
              >
              ...
              <p
                ...
                style={{
                  viewTransitionName: `portfolio-tagline-${
                    item.permalink || item.slug
                  }`,
                }}
              >
              ...
              {item.title2 && (
                <p className="font-kabel text-sm text-gray-300">
                  {item.title2}
                </p>
              )}
            </a>
        ...
        <div className="flex flex-wrap gap-2 overflow-hidden group-hover:mt-4">
          {item.category?.slice(0, 3).map((cat, index) => (
            <span
              ...
              style={{
                viewTransitionName: `portfolio-category-${
                  item.permalink || item.slug
                }-${cat}`,
              }}
            >
        ...
        <div className="flex flex-wrap gap-2 overflow-hidden group-hover:mt-4">
          {item.tag?.slice(0, 3).map((tag, index) => (
            <span
              ...
              className={`${
                index > 2 ? "hidden" : ""
              } clip2 invisible h-0 overflow-hidden border-l-2 border-purple-500/30 bg-purple-500/80 px-3 text-sm text-purple-100 transition-all hover:border-purple-500 group-hover:visible group-hover:h-7 group-hover:py-1 ${
                delay[index]
              }`}
            >
              {tag}
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      const tweetUrl = `https://x.com/intent/tweet?text=${encodeURIComponent(
                        item.title || "Check out this project"
